/**
 * Pagetrix v4.5 - Image Persistence & High-Speed Progress
 * FIX: Prevents images from flashing during page swaps by recycling cached DOM nodes.
 */
const pagetrix = (() => {
  ...
  const performSwap = (html, url) => {
    ...
    // 1. Capture current "persistent" elements and ALL images
    ...
    // 2. Perform Image Diffing BEFORE innerHTML swap
    // We find images in the newDoc that match old images by SRC
    newDoc.querySelectorAll('img').forEach(newImg => {
      const match = oldImages.find(oldImg => oldImg.src === newImg.src);
      if (match) {
        // Transfer the 'complete' state and cache to prevent the flicker
    ...
    // 4. Re-inject specific persistent nodes (Header, Player, etc)
