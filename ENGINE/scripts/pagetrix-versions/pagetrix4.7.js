/**
 * Pagetrix v4.7 - Image Persistence & High-Speed Progress
 * ADDED: pagetrix.reload() - Refetches current HTML without cache and re-swaps.
 */
const pagetrix = (() => {
  ...
  const goto = async (rawUrl, bypassCache = false) => {
  ...
  /**
   * RE-LOAD: Refetches current page without using the Pagetrix cache.
   */
  const reload = () => {
    goto(window.location.href, true);
  };
